...
SELECT empno, ename, mgr, nvl2(mgr, mgr, 9999)
FROM emp; 

SELECT empno, ename, mgr, coalesce(mgr, 9999)
FROM emp;
...
SELECT empno, ename, job, sal,
        CASE
            WHEN job = 'SALESMAN' THEN sal * 1.05 -- 참거 짓을 구분할수 있는 조건 THEN 반환할 값
...
SELECT empno, ename, hiredate, DECODE(hiredate, 
FROM emp;

SELECT empno, ename, hiredate,
        CASE
         WHEN MOD (TO_CHAR(hiredate,'YYYY'),2)= MOD (TO_CHAR(SYSDATE,'YYYY'),2) THEN '검강검진 대상자'
         ELSE '검강검진 비대상자'
...
SELECT userid, usernm, alias, reg_dt,
        CASE
         WHEN MOD (TO_CHAR(reg_dt,'YYYY'),2) = MOD (TO_CHAR(SYSDATE,'YYYY'),2) THEN '검강검진 대상자'
         ELSE '검강검진 비대상자'
         END CONTACT_TO_DOCTOR
FROM users;