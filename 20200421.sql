...
SELECT *
FROM emp
WHERE LOWER(ename) = 'smith'; 이런식으로 작성하며 안된다
WHERE ename = UPPER('smith'); 두가지 방식중에는 위에보다는 아래방식이 올바른 방법이다
==> WHERE ename = 'SMITH';
...
CONCAT 함수로 작성하기;
SELECT table_name, tablespace_name, CONCAT('start', 'end'),
       CONCAT(table_name, tablespace_name),CONCAT('SELECT * FROM',table_name) , CONCAT(CONCAT('SELECT * FROM',table_name), ';')  
FROM user_tables;
...
SELECT SUBSTR('Hello, World', 1, 5) sub,
       ...
       INSTR('Hello, World', 'o', 6) ins2, -- 6번째 칸부터 o를 찾아라
       INSTR('Hello, World', 'o', INSTR('Hello, World', 'o')+1) ins3,  -- 6번째 칸부터 o를 찾아라
       LPAD('hello', 15, '*') lp,
       RPAD('hello', 15, '*') rp,
       LPAD('hello', 15) lp2,
       RPAD('hello', 15) rp2,
...
SELECT ROUND(105.54, 1) round,
       ...
       ROUND(105.55, 0) round3, -- 소수점 첫자리에서 반올림
       ROUND(105.55, -1) round4 -- 1의 자리에서 반올림
FROM dual;
...
SELECT TO_DATE('20191231','YYYYMMDD') LASTDAY, TO_DATE('20191231','YYYYMMDD') -5 LASTDAY_BEFORE5, SYSDATE NOW, SYSDATE -3 NOWBEFORE3
FROM dual;
...
NEXT_DAY(date1, 주간일자-1~7) date1이후 등장하는 첫번째 주간일자의 날짜를 변환
...
SELECT SYSDATE, LAST_DAY(SYSDATE), LAST_DAY(SYSDATE)+1, ADD_MONTHS(LAST_DAY(SYSDATE)+1,-1),
